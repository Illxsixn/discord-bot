...
CREATE INDEX IF NOT EXISTS idx_board_games_players
    ON board_games (guild_id, status);
...
CREATE TABLE IF NOT EXISTS game_stats (
    ...
    c4_wins INTEGER DEFAULT 0,
    c4_losses INTEGER DEFAULT 0,
    c4_draws INTEGER DEFAULT 0,
    PRIMARY KEY (guild_id, user_id)
);
...
class Database:
    ...
    async def create_board_game(
        ...
        board_json: str,
        *,
        status: BoardGameStatus = BoardGameStatus.PENDING,
        current_player_id: int | None = None,
    ) -> BoardGameRecord:
        """Erstellt ein neues Brettspiel."""
        created_at = datetime.now(timezone.utc).isoformat()
        current = current_player_id if current_player_id is not None else player1_id
        cursor = await self.conn.execute(
            """
            INSERT INTO board_games (
                guild_id, channel_id, message_id, game_type,
                player1_id, player2_id, board_json, current_player_id,
    ...
    async def get_board_game(self, game_id: int) -> BoardGameRecord | None:
        """Lädt Brettspiel per ID."""
        cursor = await self.conn.execute("SELECT * FROM board_games WHERE id = ?", (game_id,))
        row = await cursor.fetchone()
        return BoardGameRecord.from_row(dict(row)) if row else None

    async def get_board_game_by_message(self, message_id: int) -> BoardGameRecord | None:
        """Lädt Brettspiel anhand der Nachrichten-ID."""
        cursor = await self.conn.execute(
            "SELECT * FROM board_games WHERE message_id = ?",
            (message_id,),
        )
        row = await cursor.fetchone()
        return BoardGameRecord.from_row(dict(row)) if row else None
    ...
    async def update_board_game(
        ...
        *,
        ...
        current_player_id: int | None = None,
        status: BoardGameStatus | None = None,
        winner_id: int | None = None,
        message_id: int | None = None,
        clear_winner: bool = False,
    ) -> BoardGameRecord | None:
        """Aktualisiert Brettspiel-Felder."""
        game = await self.get_board_game(game_id)
        if game is None:
            return None

        updates: dict[str, Any] = {}
        if board_json is not None:
            updates["board_json"] = board_json
        if current_player_id is not None:
            updates["current_player_id"] = current_player_id
        ...
        await self.conn.execute(
            f"UPDATE board_games SET {columns} WHERE id = ?",
            values,
        )
        await self.conn.commit()
        return await self.get_board_game(game_id)

    async def user_in_active_board_game(self, guild_id: int, user_id: int) -> BoardGameRecord | None:
        """Prüft, ob ein Nutzer in einem aktiven Brettspiel ist."""
        cursor = await self.conn.execute(
            """
            ...
            (
                guild_id,
                BoardGameStatus.PENDING.value,
                BoardGameStatus.ACTIVE.value,
                user_id,
                user_id,
            ),
        )
    ...
    async def get_open_board_games(self) -> list[BoardGameRecord]:
        """Lädt alle offenen Brettspiele (pending oder active)."""
        cursor = await self.conn.execute(
            """
            SELECT * FROM board_games
            WHERE status IN (?, ?)
            ORDER BY id
            """,
            (BoardGameStatus.PENDING.value, BoardGameStatus.ACTIVE.value),
        )
        rows = await cursor.fetchall()
        return [BoardGameRecord.from_row(dict(row)) for row in rows]

    async def get_open_board_games_for_guild(self, guild_id: int) -> list[BoardGameRecord]:
        """Lädt offene Brettspiele eines Servers."""
        cursor = await self.conn.execute(
            """
            SELECT * FROM board_games
            ...
            ORDER BY id
            """,
            (guild_id, BoardGameStatus.PENDING.value, BoardGameStatus.ACTIVE.value),
        )
        rows = await cursor.fetchall()
        return [BoardGameRecord.from_row(dict(row)) for row in rows]

    async def get_game_stats(self, guild_id: int, user_id: int) -> GameStatsRecord:
        """Lädt Brettspiel-Statistiken."""
        cursor = await self.conn.execute(
        ...
        row = await cursor.fetchone()
        if row is None:
            return GameStatsRecord(guild_id=guild_id, user_id=user_id)
        return GameStatsRecord.from_row(dict(row))

    async def save_game_stats(self, stats: GameStatsRecord) -> GameStatsRecord:
        """Speichert Brettspiel-Statistiken."""
        await self.conn.execute(
            """
            INSERT INTO game_stats (
                guild_id, user_id, ttt_wins, ttt_losses, ttt_draws,
                c4_wins, c4_losses, c4_draws
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(guild_id, user_id) DO UPDATE SET
                ttt_wins = excluded.ttt_wins,
                ttt_losses = excluded.ttt_losses,
                ttt_draws = excluded.ttt_draws,
                c4_wins = excluded.c4_wins,
                c4_losses = excluded.c4_losses,
                c4_draws = excluded.c4_draws
            """,
            (
                stats.guild_id,
                stats.user_id,
